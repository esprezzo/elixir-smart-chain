defmodule SmartChain.Contract do
    use GenServer
    require Logger
    require IEx
    alias SmartChain.ABI
    ...
    @doc "Installs a filter on the SmartChain node. This also formats the parameters, and saves relevant information to format event logs."
    ...
    defp init_events(abi) do
      ...
      names_and_signature_types_map =
        Enum.map(events, fn {name, v} ->
          ...
          encoded_event_signature = "0x#{SmartChain.encode_abi_event(signature)}"
    ...
    def deploy_helper(bin, abi, args) do
      constructor_arg_data =
        if arguments = args[:args] do
          ...
          if constructor_abi do
            ...
            bin <> (SmartChain.encode_abi_data(types_signature, arguments) |> Base.encode16(case: :lower))
          else
      ...
      gas = SmartChain.encode_option(args[:options][:gas])
      ...
      {:ok, tx_hash} = SmartChain.eth_send([tx])
    end
    ...
    def eth_call_helper(address, abi, method_name, args) do
      ...
      result =
        SmartChain.eth_call([
          %{
            to: address,
            data: "0x#{SmartChain.encode_abi_method_call(abi, method_name, args)}"
          }
        ])
      ...
      case result do
        {:ok, data} ->   
          case data do
            ...
            d ->  
              try do             
                ret = SmartChain.decode_abi_output(abi, method_name, data)
    ...
    def handle_cast({:uninstall_filter, filter_id}, state) do
      SmartChain.uninstall_filter(filter_id)
          ...
          Enum.map(0..(length(topics) - 1), fn i ->
            ...
            if topic do
              if is_list(topic) do
                ...
                Enum.map(topic, fn t ->
                  "0x" <> (SmartChain.encode_abi_data(topic_type, [t]) |> Base.encode16(case: :lower))
                end)
              else
                topic_type = Enum.at(topic_types, i)
                "0x" <> (SmartChain.encode_abi_data(topic_type, [topic]) |> Base.encode16(case: :lower))
              end
            else
              topic
            end
      ...
      if event_data[:fromBlock] do
        new_from_block =
          if Enum.member?(["latest", "earliest", "pending"], event_data[:fromBlock]) do
            ...
            SmartChain.encode_abi_data("(uint256)", [event_data[:fromBlock]])
          end
        ...
        new_param =
          if Enum.member?(["latest", "earliest", "pending"], event_data[key]) do
            event_data[key]
              ...
              # (SmartChain.encode_abi_data("(uint256)", [event_data[key]]) |> Base.encode16(case: :lower))
              SmartChain.to_hex(event_data[key])
          end
    ...
    defp extract_non_indexed_fields(data, names, signature) do
      Enum.zip(names, SmartChain.decode_abi_event(data, signature)) |> Enum.into(%{})
    end
        ...
        if length(log["topics"]) > 1 do
          ...
          decoded_topics =
            Enum.map(0..(length(tail) - 1), fn i ->
              ...
              SmartChain.decode_abi_data(topic_type, topic_data)
            end)
    ...
    def handle_call({:filter, {contract_name, event_name, event_data}}, _from, state) do
      ...
      {:ok, filter_id} = SmartChain.new_filter(payload)
    ...
    def handle_call({:get_filter_logs, filter_id}, _from, state) do
      ...
      # IEx.pry
      {:ok, logs} = SmartChain.get_filter_logs(filter_id)
        ...
        if logs && logs != [] do
          Enum.map(logs, fn log ->
            ...
            formatted_log =
              Enum.reduce(
                [
                  SmartChain.abi_keys_to_decimal(log, [
                    "blockNumber",
                    "logIndex",
                    "transactionIndex",
                    "transactionLogIndex"
    ...
    def handle_call({:get_filter_changes, filter_id}, _from, state) do
      ...
      {:ok, logs} = SmartChain.get_filter_changes(filter_id)
    ...
    #   {:ok, receipt} = SmartChain.get_transaction_receipt_by_hash(tx_hash)
    ...
    #           Enum.zip(
    #             event_attributes[:non_indexed_names],
    #             SmartChain.decode_abi_event(log["data"], event_attributes[:signature])
    #           )
    #           |> Enum.into(%{})
    #         if length(log["topics"]) > 1 do
    #           [_head | tail] = log["topics"]
